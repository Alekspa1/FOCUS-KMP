...
class AndroidPaySdkImpl(private val pref: SharedPrefRepository, private val context: Context) :
        ...
        return suspendCancellableCoroutine { continuation ->
            RuStorePayClient.instance.getPurchaseInteractor().getPurchases()
                .addOnSuccessListener { purchases: List<Purchase> ->
                    ...
                    val hasActivePremiumInStore =
                        stateList.contains(ProductPurchaseStatus.CONFIRMED) ||
    ...
    override suspend fun isChekedSubcrition(): Result<Boolean> {
        if (isAuthorizationInRustore()) {
            return try {
                ...
                val hasActivePremiumInStore =
                    newStatuses.contains(ProductPurchaseStatus.CONFIRMED) ||
    ...
    private suspend fun getNewPurchases(): List<Purchase> =
        suspendCancellableCoroutine { continuation ->
            RuStorePayClient.instance.getPurchaseInteractor().getPurchases()
    ...
    private suspend fun getOldPurchases(billingClient: RuStoreBillingClient): List<ru.rustore.sdk.billingclient.model.purchase.Purchase> =
        suspendCancellableCoroutine { continuation ->
